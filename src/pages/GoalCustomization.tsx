...
const GoalCustomization = () => {
  ...
  const handleStepClick = (step: number) => {
    ...
    if (step === 4) navigate(`/goals/recommendation?metric=${metricId}`);
  };
  ...
  return (
    <div className="animate-slide-in">
      <WorkflowProgress currentStep={5} onStepClick={handleStepClick} />
        ...
        <div className="xl:col-span-2 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <button
              ...
              className={cn(
                "innovare-card p-4 text-left transition-all hover:shadow-md",
                mode === "accept" && "ring-2 ring-primary innovare-glow"
              )}
            >
            ...
            <button
              ...
              className={cn(
                "innovare-card p-4 text-left transition-all hover:shadow-md",
                mode === "modify" && "ring-2 ring-primary innovare-glow"
              )}
            >
            ...
            <button
              ...
              className={cn(
                "innovare-card p-4 text-left transition-all hover:shadow-md",
                mode === "override" && "ring-2 ring-primary innovare-glow"
              )}
            >
          ...
          {mode && (
            <div className="innovare-card p-6 animate-slide-in">
              <h3 className="font-heading font-semibold text-sm text-card-foreground mb-4">
                Your Target for {metric.name}
              </h3>
              ...
              <div className="flex items-center gap-6 mb-6">
                <div className="flex-1">
                  ...
                  <div className="flex items-center gap-3">
                    <input
                      ...
                      step={0.1}
                      className={cn(
                        "w-32 h-14 text-2xl font-heading font-bold text-center rounded-xl border-2 transition-colors",
                        mode === "accept"
                          ? "bg-muted border-border text-card-foreground"
                          : "border-primary bg-card text-card-foreground focus:outline-none focus:ring-4 focus:ring-primary/20"
                      )}
                    />
                ...
                {(mode === "modify" || mode === "override") && (
                  <div className="flex-1">
                    ...
                    <input
                      type="range"
                      min={mode === "override" ? 5 : goalRecommendation.conservative}
                      max={mode === "override" ? 35 : goalRecommendation.ambitious}
                      step={0.1}
                      value={goalValue}
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Document your reasoning for this goal selection..."
                  className="w-full h-24 p-3 text-sm rounded-xl border border-border bg-card text-card-foreground resize-none focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary placeholder:text-muted-foreground"
                />
              </div>
            </div>
          )}
        ...
        <div className="space-y-4">
          ...
          {mode && (
            <button
              ...
              className="w-full bg-primary text-primary-foreground py-3 rounded-xl font-heading font-semibold text-sm hover:bg-primary/90 transition-colors innovare-glow"
            >
              Confirm & Save Goal
            </button>
          )}
